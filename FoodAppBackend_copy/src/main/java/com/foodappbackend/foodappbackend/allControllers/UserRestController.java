...
public String addToCart(
    ...
    try {
        // Step 1: Fetch the `plan_order` limit from `adminlogin`
        ResultSet adminRS = DBLoader.executeSQL("SELECT plan_details FROM adminlogin");
        if (adminRS.next()) {
            // Parse the plan details from the admin login table
            String planDetailsJson = adminRS.getString("plan_details");
            JSONObject planDetails = new JSONObject(planDetailsJson);
            int planOrderLimit = planDetails.getInt("plan_order");

            // Step 2: Count the total number of items in the cart
            ResultSet cartCountRS = DBLoader.executeSQL("SELECT COUNT(*) AS cartItemCount FROM cart");
            if (cartCountRS.next()) {
                int cartItemCount = cartCountRS.getInt("cartItemCount");

                // Step 3: Check if the total cart items exceed the plan limit
                if (cartItemCount >= planOrderLimit) {
                    // Return limit exceeded message
                    return "{\"status\":\"limit_exceeded\",\"message\":\"You cannot order more . Please contact admin to upgrade your plan.\"}";
                }

                // Step 4: If the limit is not exceeded, add the item to the cart
                ResultSet rs = DBLoader.executeSQL("SELECT * FROM cart");
                rs.moveToInsertRow();
                rs.updateString("useremail", useremail);
                rs.updateString("item_id", items_id);
                ...
                return "{\"status\":\"error\",\"message\":\"Could not retrieve cart item count.\"}";
            }
        } else {
            return "{\"status\":\"error\",\"message\":\"User not found in admin login.\"}";
        }
    } catch (Exception ex) {
        ex.printStackTrace();
        return "{\"status\":\"error\",\"message\":\"" + ex.toString() + "\"}";
